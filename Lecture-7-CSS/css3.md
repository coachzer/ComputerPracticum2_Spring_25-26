...
- **animation-fill-mode**: določa, kaj se zgodi s stilom *pred* začetkom ali *po* koncu animacije, recimo ali element ostane v končnem stanju (`forwards`) ali se vrne na začetek
...
- **transition-property**: določa, *katera lastnost* naj se animira. To je lahko na primer `background-color`, `width`, `transform` ali pa `all`, če želimo, da se animirajo vse spremembe
...
Če vse to združimo, lahko z eno samo vrstico povemo: *katera lastnost se spreminja, kako dolgo traja prehod, kdaj se začne in kako poteka*.
